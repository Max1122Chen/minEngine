...
def qualify_field_type_name(field_type: str, namespace_name: str) -> str:
    ...
    # Brought into translation units via `using namespace minEngine::Math;` in Math.h,
    # but the true nested type lives under minEngine::Math::*.
    math_type_names = {
        "Vector2": "minEngine::Math::Vector2",
        "Vector3": "minEngine::Math::Vector3",
        "Vector4": "minEngine::Math::Vector4",
        "Matrix3": "minEngine::Math::Matrix3",
        "Matrix4": "minEngine::Math::Matrix4",
    }
    ...
    elif token in math_type_names:
        qualified_core = math_type_names[token]
    else:
...
def supports_native_instance_thunks(meta: ClassMeta) -> bool:
    """Reflection native thunks cast MEObject* → TOwner*; only MEObject hierarchy is safe."""
    for base in meta.base_types:
        leaf = base.split("::")[-1]
        if leaf in ("MEObject", "Component"):
            return True
    return False
...
def has_specifier(specifiers: list[str], name: str) -> bool:
    return name in specifiers


def is_script_type_class(meta: ClassMeta) -> bool:
    return has_specifier(meta.specifiers, "ScriptType")


def script_bind_symbol(meta: ClassMeta) -> str:
...
def script_assignable_lua_name(field_name: str) -> str:
    """Strip leading m_ for ScriptAssignable Dynamic fields (CORE-F21)."""
    if field_name.startswith("m_") and len(field_name) > 2:
        return field_name[2:]
    return field_name


def is_script_assignable_property(prop: PropertyMeta) -> bool:
    return has_specifier(prop.specifiers, "ScriptAssignable")


def render_script_assignable_property(type_name: str, prop: PropertyMeta) -> str:
    """Readonly Lua property returning DynamicMulticastDelegateBase*."""
    lua_name = script_assignable_lua_name(prop.name)
    # Prefer ScriptGetter so Reflection meta Getter is not required (non-copyable Dynamic types).
    getter_name = prop.metadata.get("ScriptGetter") or prop.metadata.get("Getter")
    if getter_name:
        body = (
            f"[]({type_name}& self) -> ::minEngine::DynamicMulticastDelegateBase* "
            f"{{ return &self.{getter_name}(); }}"
        )
    else:
        body = (
            f"[]({type_name}& self) -> ::minEngine::DynamicMulticastDelegateBase* "
            f"{{ return &self.{prop.name}; }}"
        )
    return f'            "{lua_name}", sol::property({body})'
...
def render_script_binding_cpp(meta: ClassMeta, script_keys: set[str]) -> str:
    ...
    type_name = full_type_name(meta)
    needs_delegate_base = any(is_script_assignable_property(prop) for prop in meta.properties)
    lines: list[str] = [
        "// Auto-generated by minEngine_header_tool.py (ScriptBinding). Do not edit manually.",
        f'#include "{meta.source_include}"',
        '#include "Runtime/Function/Scripting/LuaScriptBindingSolTraits.h"',
    ]
    if needs_delegate_base:
        lines.append('#include "Runtime/Core/Delegates/DynamicMulticastDelegateBase.h"')
    lines.extend(
        [
            "",
            ...
            "namespace minEngine",
            "{",
            f"    void {script_bind_symbol(meta)}(sol::state& state)",
            "    {",
        ]
    )
    ...
    for prop in meta.properties:
        if is_script_assignable_property(prop):
            member_lines.append(render_script_assignable_property(type_name, prop))
        elif has_specifier(prop.specifiers, "ScriptReadWrite"):
            member_lines.append(f'            "{prop.name}", &{type_name}::{prop.name}')
        elif has_specifier(prop.specifiers, "ScriptReadOnly"):
...
def render_script_binding_register(script_classes: list[ClassMeta]) -> tuple[str, str]:
    header_lines = [
        "// Auto-generated by minEngine_header_tool.py (ScriptBinding). Do not edit manually.",
        "#pragma once",
        "",
    ...
    for meta in script_classes:
        cpp_lines.append(f"    void {script_bind_symbol(meta)}(sol::state& state);")
    cpp_lines.append("")
    cpp_lines.append("    void RegisterGeneratedLuaBindings(sol::state& state)")
    cpp_lines.append("    {")
    cpp_lines.append("        RegisterLuaScriptBindingPrimitives(state);")
    for meta in script_classes:
        cpp_lines.append(f"        {script_bind_symbol(meta)}(state);")
    cpp_lines.append("    }")
    cpp_lines.append("}")
    cpp_lines.append("")
    return "\n".join(header_lines), "\n".join(cpp_lines)
...
def emit_script_bindings(
    script_binding_out_dir: Path,
    script_binding_manifest_path: Path,
    ...
    register_header = script_binding_out_dir / "ScriptBindingRegister.gen.h"
    register_cpp = script_binding_out_dir / "ScriptBindingRegister.gen.cpp"
    header_content, cpp_content = render_script_binding_register(script_classes)
    if write_if_changed(register_header, header_content):
        generated += 1
    if write_if_changed(register_cpp, cpp_content):
        generated += 1
    referenced.add(normalize_path(register_header))
    referenced.add(normalize_path(register_cpp))

    for generated_cpp in script_binding_out_dir.rglob("*.lua_bind.gen.cpp"):
        if normalize_path(generated_cpp) not in referenced:
            generated_cpp.unlink()
            generated += 1

    for generated_file in list(script_binding_out_dir.glob("ScriptBindingRegister.gen.*")):
        if normalize_path(generated_file) not in referenced:
            generated_file.unlink()

    save_manifest(
        script_binding_manifest_path,
        {
            "tool_cache_version": TOOL_CACHE_VERSION,
            "script_types": [full_type_name(meta) for meta in script_classes],
            "generated_sources": sorted(p for p in referenced if p.endswith(".cpp")),
        },
    )
    return generated
...
def main() -> int:
    ...
    parser.add_argument(
        "--script-binding-out-dir",
        default="",
        help="Output directory for ScriptBinding codegen (optional)",
    )
    parser.add_argument(
        "--script-binding-manifest",
        default="",
        help="Path to ScriptBinding manifest json (required when --script-binding-out-dir is set)",
    )
    ...
    script_binding_out_dir = Path(args.script_binding_out_dir).resolve() if args.script_binding_out_dir else None
    script_binding_manifest_path = (
        Path(args.script_binding_manifest).resolve() if args.script_binding_manifest else None
    )
    if script_binding_out_dir is not None and script_binding_manifest_path is None:
        print("[minEngine_header_tool][ERROR] --script-binding-manifest is required with --script-binding-out-dir")
        return 2
    ...
    script_binding_generated = 0
    if script_binding_out_dir is not None and script_binding_manifest_path is not None:
        script_binding_generated = emit_script_bindings(
            script_binding_out_dir,
            script_binding_manifest_path,
            parsed_classes_by_key,
        )
    ...
    if script_binding_out_dir is not None:
        print(
            f"[minEngine_header_tool] script_binding_generated={script_binding_generated} "
            f"output={normalize_path(script_binding_out_dir)}"
        )
    return 0
