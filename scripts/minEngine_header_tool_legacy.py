...
def render_class_registration(meta: ClassMeta) -> list[str]:
    ...
    for prop in meta.properties:
        if prop.metadata:
            ...
            lines.append(f"    ME_REFLECT_FIELD({type_name}, {prop.name}, {metadata_entries})")
        else:
...
def collect_headers(src_root: Path) -> list[Path]:
    ...
    excluded_prefix = normalize_path(src_root / "Runtime/Core/Serialization/Legacy") + "/"
    return [path for path in unique_files if not normalize_path(path).startswith(excluded_prefix)]
