...
def render_enum_registration(meta: EnumMeta) -> list[str]:
    # New MEReflection path currently does not maintain standalone enum registry.
    # Enum fields are still handled by type traits as primitive-like values.
    return []
...
def render_source_gen_header(classes: list[ClassMeta], enums: list[EnumMeta]) -> str:
    ...
    lines.append("")
    lines.append('#include "Runtime/Core/Reflection/MEReflectionMacros.h"')
    lines.append("")
