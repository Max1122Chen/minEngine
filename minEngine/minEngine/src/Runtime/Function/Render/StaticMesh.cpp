...
namespace minEngine
{
    StaticMesh::StaticMesh(float *vertices, uint32_t verticesSize, uint32_t numVertices, std::initializer_list<RHIVertexElement> elements)
    {
    ...
    StaticMesh::StaticMesh(float *vertices, uint32_t verticesSize, uint32_t numVertices, std::initializer_list<RHIVertexElement> elements, uint32_t *indices, uint32_t numIndices)
    {
        m_VertexBuffer = VertexBuffer::Create(vertices, verticesSize, numVertices);
        m_VertexDefinition = VertexDefinition::Create(elements);
