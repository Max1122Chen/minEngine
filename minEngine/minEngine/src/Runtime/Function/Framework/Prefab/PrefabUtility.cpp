...
namespace minEngine
{
        ...
        const GUID& prefabAssetGuid)
    {
        ...
        for (const PrefabInstanceRecord& record : scene.GetPrefabInstances())
        {
            ...
            if (std::shared_ptr<MEObject> rootObject = ObjectManager::Get().FindObject(record.RootInstanceGuid))
            {
    ...
    std::vector<PrefabInstanceRef> PrefabUtility::FindInstanceRefsInOpenEditorScenes(const GUID& prefabAssetGuid)
    {
        std::vector<PrefabInstanceRef> refs;
        if (!SceneManager::HasInstance())
        {
