...
{
    void DebugCommandHistory::Push(std::string line)
    {
        if (line.empty())
        {
            return;
    ...
    void DebugCommandHistory::ResetNavigation()
    {
        m_NavigationIndex = -1;
        m_SavedDraft.clear();
    }

    std::optional<std::string> DebugCommandHistory::NavigateUp(const std::string& currentDraft)
    {
        if (m_Entries.empty())
        {
            return std::nullopt;
    ...
    std::optional<std::string> DebugCommandHistory::NavigateDown(const std::string& currentDraft)
    {
        (void)currentDraft;
