...
namespace minEngine::DebugCommand
{
    namespace
    {
            ...
            std::string& outError)
        {
            ...
            DebugCommandSetValueValidation::PopulatePropertyConstraints(property, valueInfo);
            const std::optional<std::string> constraintError =
                DebugCommandSetValueValidation::ValidateNumericConstraints(valueInfo, numericValue);
    ...
    bool PropertyPath::Resolve(const DebugCommandContext& context, ResolvedPropertyTarget& outTarget) const
    {
        return TryResolve(context, outTarget, nullptr) == PropertyPathResolveStatus::Ok;
    }

    DebugCommandResult PropertyPath::BuildResolveErrorResult(
        ...
        const std::vector<std::string>* ambiguousCandidates) const
    {
        ...
        if (status == PropertyPathResolveStatus::Ambiguous)
        {
            ...
            if (ambiguousCandidates != nullptr)
            {
                ...
                {
                    builder.AddSegment(DebugCommandOutputKind::Hint, "  ");
                    builder.AddLine(DebugCommandOutputKind::ListItemName, candidate);
                }
            }
    ...
    DebugCommandResult PropertyPath::GetValue(const DebugCommandContext& context) const
    {
        ...
        {
            DebugCommandOutputBuilder builder;
            builder.AddLine(DebugCommandOutputKind::Error, "Error: failed to read property value.");
            return builder.BuildError("read failed");
        }

        DebugCommandOutputBuilder builder;
        builder.AddSegment(DebugCommandOutputKind::Path, GetCanonicalPath());
        builder.AddSegment(DebugCommandOutputKind::Muted, " = ");
        builder.AddSegment(DebugCommandOutputKind::ValueLiteral, valueText);
        builder.NewLine();

        std::string payload = "{\"op\":\"get\",\"path\":";
        payload += DebugCommandPayloadJson::Quote(GetCanonicalPath());
        payload += ",\"value\":";
        payload += DebugCommandPayloadJson::Quote(valueText);
        payload += "}";
        ...
        int indentLevel)
    {
        ...
        Reflection::ReflectionSystem::Get().ForEachPropertyInHierarchy(
            ...
            [&](const Reflection::MEProperty& property) -> bool
            {
                ...
                builder.AddSegment(DebugCommandOutputKind::InspectKey, indent + Reflection::GetPropertyDisplayName(property));
                builder.AddSegment(DebugCommandOutputKind::Muted, ": ");

                if (property.GetCategory() == Reflection::MEPropertyCategory::Primitive)
                {
                    const std::string valueText = FormatPropertyValueAsText(ownerObject, ownerClass, property.GetName());
                    builder.AddSegment(DebugCommandOutputKind::InspectValue, valueText.empty() ? "<unavailable>" : valueText);
                    builder.NewLine();
                    return true;
                }
                ...
                if (property.GetCategory() == Reflection::MEPropertyCategory::Object)
                {
                    builder.AddSegment(DebugCommandOutputKind::InspectType, "<object>");
                    builder.NewLine();
                    return true;
                }
    ...
    DebugCommandResult PropertyPath::BuildSetValueSuccessResult(const DebugCommandContext& context) const
    {
        ResolvedPropertyTarget target;
        if (!Resolve(context, target))
        {
            DebugCommandOutputBuilder builder;
            builder.AddLine(DebugCommandOutputKind::Error, "Error: could not resolve property path after set.");
            return builder.BuildError("property path not found");
        }
        ...
        minEngine::EditorPropertyEditContextKind editContextKind) const
    {
        ...
        {
            DebugCommandOutputBuilder builder;
            builder.AddLine(DebugCommandOutputKind::Error, "Error: " + walkError);
        ...
        {
            DebugCommandOutputBuilder builder;
            builder.AddLine(DebugCommandOutputKind::Error, "Error: leaf property not found.");
        ...
        {
            DebugCommandOutputBuilder builder;
            builder.AddLine(DebugCommandOutputKind::Error, "Error: property is read-only.");
        ...
        {
            DebugCommandOutputBuilder builder;
            builder.AddLine(DebugCommandOutputKind::Error, "Error: failed to read current property value.");
        ...
        {
            DebugCommandOutputBuilder builder;
            builder.AddLine(DebugCommandOutputKind::Error, "Error: " + encodeError);
        ...
        {
            DebugCommandOutputBuilder builder;
            builder.AddLine(DebugCommandOutputKind::Error, "Error: invalid property owner.");
        ...
        outError = DebugCommandResult::MakeOk();
        return true;
    }
        ...
        PropertyWriteMode writeMode) const
    {
        ...
        {
            DebugCommandOutputBuilder builder;
            builder.AddLine(DebugCommandOutputKind::Error, "Error: property owner not found.");
        ...
        {
            DebugCommandOutputBuilder builder;
            builder.AddLine(DebugCommandOutputKind::Error, "Error: property owner class not found.");
    ...
    DebugCommandResult PropertyPath::Inspect(const DebugCommandContext& context) const
    {
        if (context.ActiveScene == nullptr)
        {
            DebugCommandOutputBuilder builder;
        ...
        {
            DebugCommandOutputBuilder builder;
            builder.AddLine(DebugCommandOutputKind::Error, "Error: object not found '" + m_GameObjectName + "'");
        ...
        {
            DebugCommandOutputBuilder builder;
            builder.AddLine(DebugCommandOutputKind::Error, "Error: object class unresolved.");
            return builder.BuildError("class unresolved");
        }

        DebugCommandOutputBuilder builder;
        builder.AddSegment(DebugCommandOutputKind::InspectHeader, gameObject->GetName());
        builder.AddSegment(DebugCommandOutputKind::Muted, " : ");
        builder.AddSegment(DebugCommandOutputKind::InspectType, gameObjectClass->GetName());
        builder.NewLine();
        ...
        if (!m_PropertySubPath.empty())
        {
            ...
            {
                builder.AddLine(DebugCommandOutputKind::InspectSection, Reflection::GetPropertyDisplayName(*leafProperty));
            ...
            const std::string valueText =
                FormatPropertyValueAsText(target.OwnerObject, target.OwnerClass, target.PropertySubPath);
            builder.AddSegment(DebugCommandOutputKind::InspectKey, Reflection::GetPropertyDisplayName(*leafProperty));
            builder.AddSegment(DebugCommandOutputKind::Muted, ": ");
            builder.AddSegment(DebugCommandOutputKind::InspectValue, valueText.empty() ? "<unavailable>" : valueText);
            builder.NewLine();
            return builder.BuildOk();
        ...
        if (!m_ExplicitComponentName.empty())
        {
            ...
            if (componentClass == nullptr)
            {
                builder.AddLine(DebugCommandOutputKind::Error, "Error: component class unresolved.");
                return builder.BuildError("class unresolved");
            }

            builder.AddLine(DebugCommandOutputKind::InspectSection, componentClass->GetName());
            AppendInspectProperties(builder, component, componentClass, 1);
            return builder.BuildOk();
        }

        builder.AddLine(DebugCommandOutputKind::InspectSection, "GameObject");
