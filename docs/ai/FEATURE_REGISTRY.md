...
Last updated: 2026-09-09（ANIM-F03 **In Progress**；S08b code Done）
...
| `CORE-F08` | Parameter Schema / Layout / Store（Schema→紧凑实例内存） | **Done** | — | [Design](./Platform/Core/CORE-F08_PARAMETER_STORAGE_DESIGN.md) · [Impl](./Platform/Core/CORE-F08_PARAMETER_STORAGE_IMPLEMENTATION.md) · `Function/Framework/Parameters/`；S00–S02 Done；`test parameter-store` PASS |
...
| `ANIM-F03` | Animation Graph MVP（Unity 式 FSM + Params + Transition Blend） | **In Progress** | — | [Design](./Animation/ANIM-F03_ANIMATION_GRAPH_DESIGN.md) · [Impl](./Animation/ANIM-F03_ANIMATION_GRAPH_IMPLEMENTATION.md) · Runtime `a59b79a`；S08+S08b code Done（Inspector+SM disguise）；smoke PASS；Preview Deferred；真·SM 画布另议 |
