...
- **Related:** [Implementation](./CORE-F03_TRANSFORM_QUATERNION_IMPLEMENTATION.md), [FEATURE_REGISTRY.md](../../FEATURE_REGISTRY.md), [EDITOR_APPEARANCE.md](../../Editor/EDITOR_APPEARANCE.md), [SERIALIZATION_BINARY_AND_PROPERTY_API.md](../Serialization/SERIALIZATION_BINARY_AND_PROPERTY_API.md), [PHYS-F01](../../FEATURE_REGISTRY.md)
- **Note:** 原 physics 分支登记为 `CORE-F01`；合入 `master` 时改号为 `CORE-F03`（`CORE-F01`/`CORE-F02` 已分配给 Lua）。
...
2. [CORE-F03_TRANSFORM_QUATERNION_IMPLEMENTATION.md](./CORE-F03_TRANSFORM_QUATERNION_IMPLEMENTATION.md) — 切片、PR 边界、每步命令。
