...
- **Last updated:** 2026-09-02（+ camelCase 单词空格拆分；合入 `master`）
- **Branch:** `master`（原 `feat/editor` 轨）
- **Depends on:** P4 Reflection（`MEProperty` metadata 已有 `DisplayName` 键）
- **Related:** [FEATURE_REGISTRY.md](../../FEATURE_REGISTRY.md) · [ACTIVE_WORK.md](../../ACTIVE_WORK.md) · [ED-F04 Debug Console Design](../../Editor/ED-F03_DEBUG_CONSOLE_COMMAND_SYSTEM_DESIGN.md)（`inspect` 输出共享展示名规则）
...
- [x] `FormatMemberDisplayName("m_LightColor") == "Light Color"`
- [x] `FormatMemberDisplayName("m_InnerConeAngle") == "Inner Cone Angle"`
- [x] `FormatMemberDisplayName("x_Foo") == "Foo"`；`FormatMemberDisplayName("b_Enabled") == "Enabled"`
- [x] 有 `DisplayName` metadata 时返回 metadata（不自动分词）
- [x] Inspector 目视：`LightComponent` 等字段为 `Light Color` 风格标签
- [x] Undo / Save Scene 仍用真实路径（回归：改 `m_Intensity` → Undo 描述/序列化正常）
- [x] `verify.ps1` 通过
...
| S02 | `PropertyEditPolicy` 接线 + Inspector 目视 | **Done** |
