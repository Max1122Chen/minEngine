...
Status: **拍板 v1 — M0 Color 实施中 / 其余里程碑未开工**  
...
```text
Inspector / Drawer
       → PropertyEditPolicy (Specifier, Meta, Context)
       → PropertyWidgets
              ├─ TransformWidget
...
（保持 v0 设计：`PropertyEditSession` 承载 `OnMarkDirty`、`TryBeginEdit`；`PropertyEditPolicy` 读 Specifier/Meta。）
