...
- **Related:** [Implementation](./PHYS-F01_JOLT_INTEGRATION_IMPLEMENTATION.md), [FEATURE_REGISTRY.md](../FEATURE_REGISTRY.md), [CORE-F03 Transform quaternion](../Platform/Core/CORE-F03_TRANSFORM_QUATERNION_DESIGN.md)
...
- `CORE-F03`（原 physics 分支 `CORE-F01`）已将 `Transform::Rotation` 改为四元数存储，为 Jolt pose 写回铺平道路。
