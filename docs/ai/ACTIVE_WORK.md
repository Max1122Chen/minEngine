...
- **RND-F01** RenderGraph — **Draft**（图 **`RenderPass`**；逻辑纹理 **字符串名**；`RenderPasses/*` ≈ `RenderPassInterface`）。见 [F01](./Render/RND-F01_RENDER_GRAPH_DESIGN.md)。**S0 + S01 + S02 Done**；**下一步：S03 全主帧 Pass 化**。
- **下一步主线：** F01-**S03** 或 F03-M3 尾；F03 Done / F05 仍按原依赖。
