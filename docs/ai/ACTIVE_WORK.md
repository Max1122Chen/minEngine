...
- **RND-F02** Modern RHI — **S2 词汇层 Done**（Modern 类型 + `RHIShaderLegacy` 重命名 + `RHIGraphicsPSODesc`；`verify` 绿）→ **S3**（`RHICreate*`/`RHICmd*` 契约声明）。见 [RND-F02](./Render/RND-F02_MODERN_RHI_DESIGN.md) §B.8。
