...
| TD-004 | Content Browser full tree rebuild on registry change | ED / ASSET | **Medium** | Open | [CONTENT_BROWSER_REGISTRY_REFRESH_ISSUE.md](./Platform/ContentBrowser/CONTENT_BROWSER_REGISTRY_REFRESH_ISSUE.md) · **[BUG-ASSET-001](./bugs/BUG-ASSET-001.md)** | Incremental registry OK；UI model 曾整树重建。另：watcher 目录事件可触发 **全盘 ScanAssets**（与 UI 重建不同层） |
